...
public static class SearchResponseExtensions
{
    public static ElasticSearchSearchResults ConvertToSearchResults(this SearchResponse<ElasticDocument> searchResult)
    {
        ...
        var results = searchResult.Hits.Select(x =>
            new SearchResult(x.Id, (float)x.Score.Value, () => x.Source?.ToDictionary(field=>field.Key, field=> field.Value is IEnumerable<object> list ? list.Select(item=>item.ToString()).ToList() : new List<string?>(){ field.Value.ToString()}))).ToList();
