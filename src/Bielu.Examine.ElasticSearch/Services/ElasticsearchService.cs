...
public class ElasticsearchService(
    ...
    IIndexStateService service,
    IPropertyMappingService propertyMappingService,
    IAnalyzerMappingService analyzerMappingService,
    ...
    ILoggerFactory loggerFactory) : ISearchService
{
    ...
    public void EnsuredIndexExists(string examineIndexName, string analyzer,
        ReadOnlyFieldDefinitionCollection properties, bool overrideExisting = false)
    {
        var fieldsMapping = propertyMappingService.GetElasticSearchMapping(properties, analyzer);
    ...
    public void CreateIndex(string examineIndexName, string analyzer,
        Func<PropertiesDescriptor<BieluExamineDocument>, PropertiesDescriptor<BieluExamineDocument>> fieldsMapping)
    {
        ...
        var index = client.Indices.Create(indexName, c => c
            .Settings(x => x.Analysis(a => a.Analyzers(aa =>
                analyzerMappingService.GetElasticSearchAnalyzerMapping(aa, state.Analyzer ?? analyzer))))
    ...
    public ElasticSearchSearchResults Search(string examineIndexName,
    ...
    public long IndexBatch(string? examineIndexName, IEnumerable<ValueSet> values)
    {
        ...
        var batch = ToElasticSearchDocs(values,
            state.Reindexing ? state.CurrentTemporaryIndexName : state.CurrentIndexName);
    ...
    private BulkRequestDescriptor ToElasticSearchDocs(IEnumerable<ValueSet> docs, string? indexTarget)
    {
        ...
        foreach (var d in docs)
        {
            try
            {
                ...
                descriptor = descriptor.Index<BieluExamineDocument>(ad, indexTarget,
                    indexingNodeDataArgs => indexingNodeDataArgs.Index(indexTarget).Id(ad["id"].ToString()));
            }
            catch (Exception e)
            {
